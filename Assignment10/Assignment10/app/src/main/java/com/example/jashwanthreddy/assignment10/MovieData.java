...
public class MovieData {
    ...
    public void addItem(int position, Map<String, ?> movie) {
        moviesList.add(position, movie);

    }

    public void selectall(List<Map<String, ?>> moviesList) {
    ...
    public void clearAll(List<Map<String, ?>> moviesList) {
    ...
    public void deletemovie(List<Map<String, ?>> moviesList) {
        ...
        moviesList.removeAll(toRemove);
    }


    public int findMovie(String str) {

        List<Map<String, ?>> movieList = getMoviesList();

        for (Map<String, ?> map : movieList) {
            HashMap<String, String> temp = (HashMap<String, String>) map;

            String moviename = (String) temp.get("name");
            if(moviename.toLowerCase().contains(str.toLowerCase())){
                return movieList.indexOf(moviename);
            }
    ...
    public int findFirst(String query)
    {
        for(int i = 0; i < moviesList.size(); i++)
        {
            ...
            if (movie.get("name").toString().toLowerCase().contains(query.toLowerCase()) == true)
                return i;
            if (movie.get("description").toString().toLowerCase().contains(query.toLowerCase()) == true)
                return i;
            if (movie.get("stars").toString().toLowerCase().contains(query.toLowerCase()) == true)
                return i;
        }
        return -1;
    }

    public void sortmovies(List<Map<String, ?>> moviesList) {

        Collections.sort(moviesList, new Comparator<Map<String, ?>>() {
                    @Override
                    public int compare(Map<String, ?> one, Map<String, ?> two) {
                        HashMap<String, String> temp1 = (HashMap<String, String>) one;
                        HashMap<String, String> temp2 = (HashMap<String, String>) two;
                        String s1 = temp1.get("name");
                        String s2 = temp2.get("name");
    ...
    public void sortmoviesonyear(List<Map<String, ?>> moviesList) {
        ...
        Collections.sort(moviesList, new Comparator<Map<String, ?>>() {
                    ...
                    public int compare(Map<String, ?> one, Map<String, ?> two) {
                        HashMap<String, String> temp1 = (HashMap<String, String>) one;
                        HashMap<String, String> temp2 = (HashMap<String, String>) two;
                        String s1 = temp1.get("year");
                        String s2 = temp2.get("year");
                        Integer i1 = Integer.parseInt(s1);
                        Integer i2 = Integer.parseInt(s2);
                        if (i1 >= i2) {
                            return -1;
